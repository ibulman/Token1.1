...
public class CalendarView: UIView {
  ...
  public init() {
    ...
    if newWindow == nil {
      ...
      contentView.removeObservers()
        ...
        NotificationCenter.default.addObserver(self, selector: "dateSelected:", name: NSNotification.Name(rawValue: CalendarSelectedDayNotification), object: nil)
    }
  }
  ...
  func dateSelected(notification: NSNotification) {
    if let date = notification.object as? NSDate {
        delegate?.calendarDidSelectDate(date: moment(date as Date))
    }
  }
